...
func (s *cql2ATSSuite) forEachQueryableConstructArithmeticExpressions() error {
	...
	s.executedByStep = true
	for _, queryable := range atsFixtureQueryablesOfTypes(api.PropertyTypeNumber, api.PropertyTypeInteger) {
		filters := []string{
			queryable.Name + " + 1 > 0",
...
func (s *cql2ATSSuite) evaluateQueryableValueComparisonTemplate(template string) error {
	...
	for _, queryable := range atsFixtureQueryablesOfTypes(api.PropertyTypeString, api.PropertyTypeBoolean, api.PropertyTypeNumber, api.PropertyTypeInteger, api.PropertyTypeTimestamp, api.PropertyTypeDate) {
...
func (s *cql2ATSSuite) evaluateStringPredicateTemplate(template string) error {
	...
	s.executedByStep = true
	for _, queryable := range atsFixtureQueryablesOfTypes(api.PropertyTypeString) {
...
func (s *cql2ATSSuite) fixturePredicateApplies(predicate atsFixturePredicate) bool {
	...
	case basicTestDataATSID:
		return predicate.Conformance == api.ConformanceBasicCQL2
	case advancedTestDataATSID:
		return predicate.Conformance == api.ConformanceAdvancedComparisonOperators
	case caseiTestDataATSID:
		return predicate.Conformance == api.ConformanceCaseInsensitiveComparison
	case accentiTestDataATSID:
		return predicate.Conformance == api.ConformanceAccentInsensitiveComparison
	case basicSpatialTestDataATSID, basicSpatialPlusTestDataATSID, spatialTestDataATSID:
		return predicate.Conformance == api.ConformanceSpatialFunctions
	case temporalTestDataATSID:
		return predicate.Conformance == api.ConformanceTemporalFunctions
	case propertyPropertyTestDataATSID:
		return predicate.Conformance == api.ConformancePropertyProperty
	case arithmeticTestDataATSID:
		return predicate.Conformance == api.ConformanceArithmetic
	default:
		return false
	}
}
...
func atsFixtureQueryablesForATSName(name string) []atsFixtureQueryable {
	...
	case "Number or Integer":
		return atsFixtureQueryablesOfTypes(api.PropertyTypeNumber, api.PropertyTypeInteger)
	case "String":
		return atsFixtureQueryablesOfTypes(api.PropertyTypeString)
	case "Boolean":
		return atsFixtureQueryablesOfTypes(api.PropertyTypeBoolean)
	case "Timestamp":
		return atsFixtureQueryablesOfTypes(api.PropertyTypeTimestamp)
	case "Date":
		return atsFixtureQueryablesOfTypes(api.PropertyTypeDate)
	default:
		return nil
	}
}
