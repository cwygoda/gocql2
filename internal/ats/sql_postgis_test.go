...
func atsFixturePostGISSQLProperties() []cql2sql.Property {
	props := make([]cql2sql.Property, 0, len(atsFixture.Queryables))
	for _, queryable := range atsFixture.Queryables {
		props = append(props, cql2sql.Property{Name: queryable.Name, Type: queryable.Type, Expr: cql2sql.Column(queryable.Name)})
	}
	return props
}
...
func postGISQueryIDs(ctx context.Context, db *sql.DB, filter string, parser *gocql2.Parser, sqlOpts []cql2sql.Option) ([]string, error) {
	...
	where, err := cql2sql.ToSQL(expr, cql2sql.PostGISDialect(), sqlOpts...)
	if err != nil {
		return nil, err
	}
	// Generated cql2sql.SQL is a trusted package output; all user CQL2 literals are bind args.
