...
// Suite. Keep this file free of SQL snippets and database-specific types so the
...
var atsFixture = atsFixtureDataSource{
	...
	Queryables: []atsFixtureQueryable{
		{Name: "name", Type: PropertyTypeString, Description: "Local place name, preserving accents where commonly used."},
		{Name: "name_ascii", Type: PropertyTypeString, Description: "ASCII/transliterated place name."},
		{Name: "country", Type: PropertyTypeString, Description: "Country name."},
		{Name: "capital", Type: PropertyTypeBoolean, Description: "Whether the place is a national capital."},
		{Name: "population", Type: PropertyTypeInteger, Description: "Approximate municipal/metropolitan population used as stable ATS data."},
		{Name: "elevation_m", Type: PropertyTypeNumber, Description: "Approximate elevation in metres above sea level; nullable for IS NULL tests."},
		{Name: "founded_date", Type: PropertyTypeDate, Description: "Commonly cited founding date, or first day of cited founding year when only a year is stable."},
		{Name: "last_updated", Type: PropertyTypeTimestamp, Description: "Stable fixture timestamp for temporal instant tests."},
		{Name: "record_start", Type: PropertyTypeTimestamp, Description: "Start timestamp of the fixture source-validity interval."},
		{Name: "record_end", Type: PropertyTypeTimestamp, Description: "End timestamp of the fixture source-validity interval."},
		{Name: "source_start", Type: PropertyTypeDate, Description: "Start date of the fixture source-validity interval."},
		{Name: "source_end", Type: PropertyTypeDate, Description: "End date of the fixture source-validity interval."},
		{Name: "geom", Type: PropertyTypeGeometry, Description: "Representative WGS84 point geometry."},
		{Name: "tags", Type: PropertyTypeArray, Description: "Real-world thematic tags such as capital, port, unesco, university, finance."},
	},
	Rows: []atsFixtureRow{
		{
			ID: "lodz",
	...
	Predicates: []atsFixturePredicate{
		{Conformance: ConformanceBasicCQL2, Filter: "country = 'Poland'", WantIDs: []string{"krakow", "lodz"}},
		{Conformance: ConformanceBasicCQL2, Filter: "capital = true", WantIDs: []string{"bogota", "luxembourg"}},
		{Conformance: ConformanceBasicCQL2, Filter: "population >= 10000000", WantIDs: []string{"istanbul", "sao_paulo"}},
		{Conformance: ConformanceBasicCQL2, Filter: "elevation_m IS NULL", WantIDs: []string{"alesund"}},
		{Conformance: ConformanceAdvancedComparisonOperators, Filter: "name_ascii LIKE 'M%'", WantIDs: []string{"montreal", "munich"}},
		{Conformance: ConformanceAdvancedComparisonOperators, Filter: "population BETWEEN 1000000 AND 2000000", WantIDs: []string{"montreal", "munich"}},
		{Conformance: ConformanceAdvancedComparisonOperators, Filter: "country IN ('Poland', 'Norway')", WantIDs: []string{"alesund", "krakow", "lodz"}},
		{Conformance: ConformanceCaseInsensitiveComparison, Filter: "CASEI(name_ascii) = casei('munich')", WantIDs: []string{"munich"}},
		{Conformance: ConformanceAccentInsensitiveComparison, Filter: "ACCENTI(name) = accenti('Lodz')", WantIDs: []string{"lodz"}},
		{Conformance: ConformanceAccentInsensitiveComparison, Filter: "ACCENTI(name) LIKE accenti('A%')", WantIDs: []string{"a_coruna", "alesund"}},
		{Conformance: ConformanceSpatialFunctions, Filter: "S_INTERSECTS(geom, BBOX(5,47,12,52))", WantIDs: []string{"luxembourg", "munich", "zurich"}},
		{Conformance: ConformanceTemporalFunctions, Filter: "T_AFTER(last_updated, TIMESTAMP('2024-03-01T00:00:00Z'))", WantIDs: []string{"a_coruna", "alesund", "bogota", "istanbul", "luxembourg", "montreal"}},
		{Conformance: ConformanceArrayFunctions, Filter: "A_OVERLAPS(tags, ('port','capital'))", WantIDs: []string{"a_coruna", "alesund", "bogota", "istanbul", "luxembourg", "montreal"}},
		{Conformance: ConformancePropertyProperty, Filter: "population > elevation_m", WantIDs: []string{"a_coruna", "bogota", "istanbul", "krakow", "lodz", "luxembourg", "montreal", "munich", "sao_paulo", "zurich"}},
		{Conformance: ConformanceArithmetic, Filter: "population / 1000000 >= 10", WantIDs: []string{"istanbul", "sao_paulo"}},
	},
}
...
func atsFixtureQueryablesOfTypes(types ...PropertyType) []atsFixtureQueryable {
	wanted := make(map[PropertyType]struct{}, len(types))
